...
@router.post("/ListTenderCommittee", response_model=List[schemas.ListTenderCommittee], )
...
@router.get("/UserInformation/Get/{account_name}", response_model=schemas.UserInformation, )
...
@router.post("/TenderCommitteeRequest/Insert", response_model=u_schemas.ReturnSuccess)
async def insert_tender_committee_request(request: schemas.FormRequest, current_user: UserBase = Depends(get_current_user),
                                    db: Session = Depends(get_db)):
    tcr = tender_committee.create_tender_committee_request(request, current_user, db)
    return {'is_success': True,
            'data': {'request_no': tcr.request_no},
            "message": None
            }


@router.get("/TenderCommitteeRequest/Get/{request_no}", response_model=schemas.TenderCommitteeRequest)
...
@router.get("/TenderCommitteeRecommendation/Get/{request_no}", response_model=List[schemas.TenderCommitteeRecommendationAll])
...
@router.patch("/TenderCommitteeRequest/Update/{request_no}", response_model=u_schemas.ReturnSuccess)
